...
try.
    ...
    convert time STAMP utc_timestamp
            tIME ZONE user_timezone into time DATA(user_time2).
...
out->write( |Horario Sistema { user_time2 time = ISO  } { user_timezone }| ).
out->write( |Horario Local { user_time2 time = user  } { user_timezone }| ).
